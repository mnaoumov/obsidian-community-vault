...
Пока ваш `Pull request` не рассмотрен автором основного хранилища и активен, вы можете добалять новые изменения, делать `Commit` и эти изменения будут автоматически присоединены к активному `PR`, повторно отправлять его не потребуется.
